...
instance.web.CrashManager = instance.web.Class.extend({
    ...
    show_error: function(error) {
        ...
        var debug_mode = jQuery.deparam(jQuery.param.querystring()).debug !== undefined; 
        var buttons = {};
        buttons[_t("Ok")] = function() {
            $(this).dialog("close");
        };
        var dialog = new instance.web.Dialog(this, {
            title: "OpenERP encountered a technical problem",
            width: '644x',
            height: debug_mode ? '500px' : '260px',
            buttons: buttons
        }).open();
        dialog.$el.html(QWeb.render('CrashManager.friendly_error', {session: instance.session, error: error}));

        function toggle_technical_details(){
            dialog.$('.oe_technical').toggleClass('oe_light');
            dialog.$('.oe_box_toggle').toggleClass('oe_open');
            dialog.$('.oe_traceback_box').toggleClass('oe_hidden');
        }

        dialog.$('.oe_technical').click(toggle_technical_details);
        
        if(debug_mode){
            toggle_technical_details();
        }
    ...
    show_common: function() {
        ...
        window.onerror = function (message, file, line) {
            self.crashmanager.show_error({
                type: _t("Uncaught JavaScript exception"),
                message: message,
